...
def replace_url(text):
    replaced_text = re.sub(
        "(http[s]?://)?((www)\.)?([a-zA-Z0-9]+)\.{1}((com)(\.(cn))?|(org))",
        url_replacer,
        text,
    )
...
def text_tokenizer(
    text, replace_url_flag=True, tokenize_sent_flag=True, create_vocab_flag=False
):
...
# how to read the dataset without tokenizing, just splitting
# idea: just use the append commands so that its split
def read_dataset(file_path, prompt_id, score_index=6, char_level=False):
    ...
    max_sentlen = -1
    with codecs.open(file_path, mode="r", encoding="UTF8") as input_file:
        next(input_file)  # skips first line (headers)
        for line in input_file:
            tokens = line.strip().split("\t")
...
if __name__ == "__main__":
    ...
    paths = [
        "./../data-set/asap/fold_2/train.tsv",
        "./../data-set/asap/fold_2/dev.tsv",
